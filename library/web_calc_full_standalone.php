...
EOD;
	...
	echo $str;	
		...
		$this->displayAccountsReceivable();
		...
		foreach($allExpDetails as $item)
		{
			...
			for($i = 1; $i < 4; $i++) {
				...
				if($i>1) {
					...
					//$this->yearlyexpenses[$counter][$i-1];
	...
	protected function renderAccountsReceivable(){
		...
		$this->displayTotal($this->monthlyaccountsreceivable,"Total Accounts Available ");
		...
		*/
		...
		$this->displayTotal($cashcollected[0],"Current");
		...
		for($i = 1; $i < 13; $i++) {
			...
			$this->displayTotal($cashcollected[$i],$i);
	...
	protected function displayAccountsReceivable()
	{
		
		
		$this->displayTotal($this->monthlytotalsales, "Total Sales");
		$this->displayTotal($this->monthlyreceivabletotalcashcollected, "Total Cash Receive");
		$this->displayTotal($this->truemonthlyaccountsreceivable, "Monthly Account Receivable");
		$this->displayTotal($this->monthlyaccountsreceivable, "Total Account Receivable");
	...
	protected function renderBalanceSheet(){
		...
		echo "g43:" . $this->yearlycash[1] . '<br>';
		echo "g44:" . $this->yearlyaccountsreceivable[1] . '<br>';
		echo "revenue 2:" . $this->yearlyrevenue[2] . '<br>';
		echo "loans k84:" . $this->yearlyamountreceive[2] . '<br>';
		echo "Budget!J51" . $this->yearlytotalmajorpurchases[2] . '<br>';
		echo "Balance Sheet'!G56" .  $this->yearlyaccountspayable[1] . '<br>';
		echo "'P & L'!J70" .  $this->yearlytotalexpenses[2] . '<br>';
		
		
		highlight_string(var_export($this->yearlyrepayments, TRUE));
	...
	public function displayBalanceSheet()
	{
		...
		$this->displayTotal($this->monthlypaidincapital, "Paid in capital");
		...
		$this->displayTotal($this->yearlynetinvestment, "Paid in capital");
	...
	protected function displayLoans()
	{
		...
		//$this->displayTotal($this->yearlyamountrepaid, "Repayment");yearlyrepayments
		$this->displayTotal($this->yearlyrepayments, "Repayment");
		$this->displayTotal($this->yearlyinterest, "Interest");
		
		
	}
	
	protected function displayInvestments()
	{
		echo "Investments Monthly Detail <br>";
			
		$this->display($this->monthlyinvestments);
		$this->displayTotal($this->monthlytotalinvestmentsamountreceive, "Total Amount Receive");
	
	
		echo "Estimated Repayments <br>";
	
		$this->display($this->monthlyinvestmentsrepaid);
		$this->displayTotal($this->monthlytotalinvestmentsamountrepaid, "Total Amount Repaid");
		
	
		echo "Yearly<br>";
	
		$this->display($this->yearlyinvestment);
		$this->displayTotal($this->yearlynetinvestment, "Net Investment");
