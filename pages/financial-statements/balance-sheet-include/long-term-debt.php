				...
				for($e_year = 0; $e_year < $_numberOfFinancialYrForcasting; $e_year++ )
				{
					if((count($grossMargin) > 0) and (count($allExpense)))
					{
						...
						<span class="cell data column-1 singleline">
							<p class="overflowable"><?php echo $open_bracket;?><?php echo $sales->defaultCurrency . number_format(($total_long_term_assets[$e_year] * $cancelNegative), 0, '.', ',')  . $closed_bracket;?></p>	
						</span>
					<?php 
					}
					else
